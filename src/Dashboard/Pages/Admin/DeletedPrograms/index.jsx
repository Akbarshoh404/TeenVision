...
const DashboardAdminDeletedPrograms = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      localStorage.setItem("deletedPrograms", JSON.stringify(uniqueItems));
    } catch (err) {
      setError(err.message);
      const storedItems = JSON.parse(
        localStorage.getItem("deletedPrograms") || "[]"
      );
  ...
  const handleRestore = useCallback(
    async (item, e) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to restore ${item.type.toLowerCase()}: ${response.status}`
          );
        }
    ...
    (e) => {
      ...
      setSortCriteria((prev) => {
        ...
        if (newCriteria.status) {
          filtered = filtered.filter((p) => p.status === newCriteria.status);
        }
          ...
          <section className={styles.section}>
            <div className={styles.container}>
              <h2 className={styles.sectionTitle}>
                Loading Deleted Programs and Tutorials...
              </h2>
            </div>
          </section>
        </main>
        ...
        <section className={styles.section}>
          <div className={styles.container}>
            <div className={styles.header}>
              <h2 className={styles.sectionTitle}>
                Deleted Programs and Tutorials
              </h2>
            </div>
            ...
            <div className={styles.cards}>
              {items.length === 0 ? (
                <p className={styles.noPrograms}>
                  {allItems.length > 0
                    ? "No matching deleted programs or tutorials."
                    : "No deleted programs or tutorials available."}
                </p>
              ) : (
                items.map((item) => (
                  <div
